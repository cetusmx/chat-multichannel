...
const regexBtn = /<button\s*onClick=\{\(e\) => \{\s*e\.preventDefault\(\);\s*setCartQty\(1\);\s*setAddToCartModal\(msg\);\s*\}\}[\s\S]*?<\/button>/m;
...
code = code.replace(regexBtn, newBtnAndInput);

fs.writeFileSync('frontend/src/features/chat/components/MessageList.jsx', code);
console.log('Card input safely injected alongside modal');
