...
// 3. Find the old button and replace it precisely using indexOf
const startSearch = 'updateClientCart(conv.client.id, newCartData)';
let buttonStartIdx = code.indexOf(startSearch);
if (buttonStartIdx > -1) {
  // Find the exact <button that encloses this logic.
  // Actually, the button starts much earlier: onClick={(e) => {
  const onClickIdx = code.lastIndexOf('<button', buttonStartIdx);
  const buttonEndIdx = code.indexOf('</button>', buttonStartIdx) + 9;
  
  const oldBtnStr = code.substring(onClickIdx, buttonEndIdx);
  ...
  console.log('Button patched safely');
} else {
  console.log('Could not find start search string');
}
