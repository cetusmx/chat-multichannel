...
const safeRender = (val) => {
  ...
  try { return JSON.stringify(val); } catch(e) { return 'Unrenderable Object'; }
};
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-gray-500 mt-2">{safeRender(error)}</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full lg:flex-row gap-6">
      <div className={`${masterClass} flex-1 lg:w-1/3 xl:w-1/4 flex-col space-y-4 overflow-y-auto pr-2 pb-10`}>
        {Array.isArray(data) ? data.map((item, i) => (
          <ResultCard 
            key={item && item.id ? `${item.type}-${item.id}` : i} 
            item={item} 
            isActive={selectedChatId === (item && item.conversationId) && targetMessageId === (item && item.id)}
            onClick={handleCardClick}
          />
        )) : <div className="text-red-500">Data is not an array: {safeRender(data)}</div>}
        ...
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-200">
          <button 
            ...
            onClick={() => typeof onPageChange === 'function' && onPageChange(pagination.page - 1)}
            className="px-3 py-1 text-sm bg-white border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50"
          >
            Anterior
          </button>
          <span className="text-sm text-gray-500">Página {safeRender(pagination?.page || 1)}</span>
          <button 
