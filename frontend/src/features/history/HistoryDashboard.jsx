            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950">
              {selectedChat?.messages?.length > 0 ? (
                selectedChat.messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.id || msg._id || idx} className={`flex flex-col max-w-[85%] ${isVendor ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                      <div className={`px-4 py-2 rounded-2xl ${isVendor ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none'} shadow-sm`}>
