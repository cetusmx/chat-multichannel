...
const regex = /await tx\.conversation\.update\(\{\s*where: \{ id: conversationId \},\s*data: dataToUpdate\s*\}\);/;

const replacement = `await tx.conversation.update({
...
if (code.match(regex)) {
  ...
  console.log('Fixed chat routes for footprint');
} else {
